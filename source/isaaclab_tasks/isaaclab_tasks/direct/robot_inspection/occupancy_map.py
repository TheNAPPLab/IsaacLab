...
class OccupancyMap2D():
    def __init__(self, x_lower, y_lower, x_upper, y_upper, resolution=0.05, dynamic_z = False,
        ...
        self.dynamic_z = dynamic_z

        self.z_margin_low = -0.2   # meters below camera height
        self.z_margin_high = 0.3   # meters above camera height
    ...
    def update_map(self, point_cloud_3D, robot_position):
    ...
    def display_map(self, robot_position=None):
        ...
        img = np.full((self.y_width, self.x_width, 3), self.COLOR_UNKNOWN, dtype=np.uint8)


        img[prob_map_transposed > self.prob_occ_thresh] = self.COLOR_OCCUPIED
        img[prob_map_transposed < self.prob_free_thresh] = self.COLOR_FREE
        if robot_position is not None:
            gx, gy = self._world_to_grid(robot_position[0], robot_position[1])
            if self._is_in_bounds(gx, gy):
                # Draw a red circle for the robot
                cv2.circle(img, (gx, gy), radius=4, color=self.COLOR_ROBOT, thickness=-1)

        # Show the map in an OpenCV window
        cv2.imshow("2D Occupancy Grid Map", cv2.flip(img, 0))
        cv2.waitKey(1)